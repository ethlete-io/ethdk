...
- **No function calls in value bindings except signal reads.** A method call in a
  ...
  (This is *not* lint-enforced.)

```html
<!-- ❌ runs every CD cycle -->  <button [disabled]="isDisabled()">
<!-- ✅ computed signal -->      <button [disabled]="disabled()">
```
...
- Inline template/styles for small components; external `.html` / `.css` files
  for complex ones.
- Building a **library** component in `libs/components`? Follow the three-tier
  model in the **`component-architecture`** skill.
