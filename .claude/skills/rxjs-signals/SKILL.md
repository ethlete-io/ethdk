...
const count$ = new BehaviorSubject(0);   const count = signal(0);

// ❌ copy an observable into state       // ✅ bridge
let data; obs$.subscribe((d) => data = d);   const data = toSignal(obs$);
```
...
toObservable(page)
  .pipe(switchMap((p) => fetchPage(p)), tap(handle), takeUntilDestroyed())
  .subscribe();
```
