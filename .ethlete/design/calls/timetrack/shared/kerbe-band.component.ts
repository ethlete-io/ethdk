...
/**
 * Draws one band at the height its minutes buy, under the treatment and the separator, shrink and
 * narrow answers the call is testing.
 */
export const kerbeBand = ({
  band,
  treatment,
  separator = 'edge',
  shrink = 'plain',
  ...
  band: Band;
  treatment: BandTreatment;
  separator?: BandSeparator;
  shrink?: BandShrink;
  narrow?: BandNarrow;
  alt?: boolean;
  marked?: boolean;
  ...
  const heightRem = (band.minutes / 60) * HOUR_REM;
  const showsDetail = heightRem >= DETAIL_MIN_REM;
  const compact = heightRem < COMPACT_MAX_REM;
  const showsTime = shrink === 'timed' || heightRem >= TIME_MIN_REM;
  const minutes = band.minutes;
  const duration = minutes < 60 ? `${minutes}m` : `${Math.floor(minutes / 60)}h ${minutes % 60}m`.replace(' 0m', '');
    ...
    <div class="band-host">
      ...
      >
        ...
        <div class="band__head">
          ...
          ${showsTime && html`<span class="band__time">${duration}</span>`}
        </div>
        ${showsDetail && band.detail && html`<span class="band__detail">${band.detail}</span>`}
      </div>
    </div>
  `;
};

/** The styles `kerbeBand` needs, for a drawing to include in its own `styles`. */
export const KERBE_BAND_STYLES = css`
  .band-host {
    display: block;
