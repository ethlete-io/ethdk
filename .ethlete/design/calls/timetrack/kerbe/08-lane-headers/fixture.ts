...
/** What a lane header carries, above the column it names. */
export type HeadRule =
  | 'name'
  | 'total'
  | 'share'
  | 'asks'
...
export type LaidLane = {
  ...
  minutes: number;
  total: string;
  /** The lane's share of the day's work, 0 to 1, for the bar under the header. */
  share: number;
  /** Bands in the lane that ask the reader for something. */
  asks: number;
};
...
const DAY_MINUTES = LANE_MINUTES.reduce((sum, minutes) => sum + minutes, 0);
...
export const LANES: LaidLane[] = WORK_SOURCE.map((lane, i) => {
  ...
  return {
    ...
    share: DAY_MINUTES ? minutes / DAY_MINUTES : 0,
    asks: lane.bands.filter((band) => band.ask !== 'nothing').length,
