import { Band, DayLaneFixture, FULL_DAY_LANES } from '../../shared/kerbe';
...
/** How wide the clock column is, and what the label in it says. */
export type GutterRule = 'today' | 'hour-padded' | 'hour-bare' | 'roomy';

export type GutterSpec = {
  gutterRem: number;
  labelRem: number;
  labelInk: string;
...
const padded = (hour: number) => String(hour).padStart(2, '0');

export const GUTTER_SPECS: Record<GutterRule, GutterSpec> = {
  today: { gutterRem: 5, labelRem: 1.05, labelInk: 'var(--k-ink-3)', label: (hour) => `${padded(hour)}:00` },
  'hour-padded': { gutterRem: 3.4, labelRem: 1.05, labelInk: 'var(--k-ink-3)', label: padded },
  'hour-bare': { gutterRem: 2.6, labelRem: 1.05, labelInk: 'var(--k-ink-3)', label: (hour) => `${hour}` },
  roomy: { gutterRem: 6.4, labelRem: 1.3, labelInk: 'var(--k-ink-2)', label: (hour) => `${padded(hour)}:00` },
};
...
export type LaidLane = {
  ...
  widthRem: number;
  laid: Laid[];
  /** Minutes of work in the lane, and how they read as a clock. */
  minutes: number;
  total: string;
};
...
/** A step of the grid the app books in, between one hour and the next. */
export type LaidRung = { key: string; topRem: number; half: boolean };
...
export const RUNGS: LaidRung[] = HOURS.slice(0, -1).flatMap((hour) =>
  [15, 30, 45].map((minute) => ({
    key: `${hour.hour}-${minute}`,
    topRem: hour.topRem + (minute / 60) * HOUR_REM,
    half: minute === 30,
  })),
);

const LANE_MINUTES = WORK_SOURCE.map((lane) => lane.bands.reduce((sum, band) => sum + band.minutes, 0));

export const LANES: LaidLane[] = WORK_SOURCE.map((lane, i) => {
  const minutes = LANE_MINUTES[i] ?? 0;
