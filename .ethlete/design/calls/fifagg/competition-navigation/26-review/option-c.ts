...
const row = ({ narrow = false, long = false }: { narrow?: boolean; long?: boolean }) => html`
  <nav class="sub">
    <div class="links">
      <a class="is-current">${CURRENT}</a>
      ${stage(long)} ${(narrow ? PAGES.slice(1, 4) : PAGES.slice(1)).map((page) => html`<a>${page}</a>`)}
...
/** The stage joins the link row as the entry after Overview, with no border and no fill. */
