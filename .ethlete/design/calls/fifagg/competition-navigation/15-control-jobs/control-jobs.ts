...
import { COMPETITION, CURRENT, LIVE, PAGES, REST_PAGES, STAGE_COUNT, VISIBLE_PAGES } from './fixture';

/** What each control on the right of the row does. */
export type JobRule = 'link-and-menu' | 'split-and-menu' | 'split-and-overflow';
...
const liveLink = html` <a class="ctl ctl--live"><span class="dot"></span><span>${LIVE.name}</span></a> `;
...
const competitionButton = html`
  <button class="ctl" type="button">
    ...
    <i class="ctl__count">${PAGES.length + STAGE_COUNT}</i>
    <em>⌄</em>
  </button>
`;
...
const right = (rule: JobRule) => {
  if (rule === 'link-and-menu') return html`${liveLink} ${competitionButton}`;
  if (rule === 'split-and-menu') return html`${liveSplit} ${competitionButton}`;

  return html`${liveSplit} ${overflowButton}`;
};

/**
 * The chosen row, with every page of the competition in the strip. Only `rule` changes: what the
 * live control and the control beside it actually do.
 */
export const controlJobs = ({ rule }: { rule: JobRule }) =>
  drawing({
    body: html`
