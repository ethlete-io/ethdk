 ...
 *
 * Usage:
 *   node tools/treeshake/measure-bundle.mjs [--dist <dist/libs>] [--entries <file.json>]
 *                                           [--cache <dir>] [--label NAME] [--external] [--json]
 *
...
const measure = async (name, code, aliases, workDir) => {
  ...
  const result = await esbuild.build({
    ...
    minify: true,
    alias: aliases,
    plugins: flag('external') ? [externalizeNonEthlete] : [],
  });
...
const main = async () => {
  ...
  if (flag('json')) {
    console.log(JSON.stringify({ label, dist: distLibs, cache: cacheDir, external: flag('external'), rows }, null, 2));
    return;
  }
  const w = Math.max(...rows.map((r) => r.name.length), 5);
  console.log(`\n${label}${flag('external') ? '  (ethlete-only)' : '  (full bundle)'}`);
