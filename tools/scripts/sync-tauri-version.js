/**
 * Writes each Tauri app's version into the files its build reads.
 *
 * The app's `package.json` is the source. It is a private workspace package, so
...
const apps = [
  { dir: 'timetrack', crate: 'timetrack' },
  { dir: 'ethlete-studio', crate: 'ethlete-studio' },
];
...
if (stale.length) {
  console.error(`A Tauri app version is stale:\n${stale.map((line) => `  ${line}`).join('\n')}`);
