...
**Built on 2026-09-16.** `PresenceStatement` and `DayReviewEdits.statements` are in the library, the
writers are in `review/statements.ts`, and `breaksBetweenRows` applies them. On the screen a press on
a break states `present`, a range drawn in the break lane states `away`, and the day's notes take a
statement back. Proofs 4, 5 and 6 are in `review/statements.spec.ts`, and the e2e flows are in
`apps/timetrack-e2e/src/break-statement.spec.ts`.
...
The totals follow the statements too. `StreamDay.presence` carries the windows the day measured,
`statedPresence` in `review/statements.ts` unions a `present` statement in and takes an `away`
statement out, and the day screen reads its footer ratio against that. The debug totals and the
sentence in the day's notes keep the measured number, so the correction stays visible.
...
- The day's `present` total and its engagement ratio follow the statements too. The app is a human
  ...
  measured total stays in the day's notes, so the correction is visible. Tom took this point, and it
  is built.
