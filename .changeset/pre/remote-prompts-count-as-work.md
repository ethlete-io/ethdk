...
is drawn but not booked. The allowance books only on the row of the session the prompt was sent to, so
parallel lanes over the same stretch never book it twice. A prompt counts as remote only when the app
watched the seat through it; days without the input signal do not change.
