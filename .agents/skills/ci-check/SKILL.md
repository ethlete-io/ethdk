---
name: ci-check
description: Run the same checks CI runs, locally, before pushing - format, changesets, lint, test, build, bundle-size goldens and the Storybook build. Use when the user says "run CI", "ci check", "lint format test build", or before pushing a change to a published lib.
---
...
yarn lint:changesets                      # 2. unreleased changeset notes: ≤40 words, 1 paragraph, ≤3 bullets
yarn nx run-many -t lint                  # 3. ESLint, incl. @nx/dependency-checks
yarn nx run-many -t test                  # 4. all unit tests
yarn nx run-many -t build                 # 5. all libs + apps (docs build fails on dead links)
yarn nx run treeshake:bundle-goldens      # 6. bundle-size goldens
yarn nx run playground:build-storybook:ci # 7. Storybook production build
```

Step 7 is the slowest by far. Skip it only when the change touches no component source
