...
`.agents/skills/ethlete-figma-export/dump-figma-layers.py`:
...
- **Figma's own measurement overlays.** Blue annotations like `396 × 401 Hug` or `347 × 23`
  are authoritative and frequently *absent* from the CSS — `Hug` in particular tells you a
...
- **Sub-comments carry the real properties.** Figma writes `/* Auto layout */`,
  ...
  `/* Brand/Chalk White/500 */` *between* a layer's declarations. A parser that starts a new
...
- **Frame labels lie.** A frame *named* "Widget 636px" routinely has `width: 640px`, and a
...
- **Absolute `left`/`top`** are canvas coordinates of the whole board. Only the *differences*
...
- **Never authoritative: colour.** Backgrounds, text, borders and interaction states resolve
  ...
  export is information about the *designer's* palette, not a value to paste. Where the
...
**production stylesheet** and read the computed boxes back. `.agents/skills/ethlete-figma-export/measure-template.mjs`
...
- **Never lay the probes out in a flex *row*.** They shrink, and container queries then report
