...
- **`@internal` JSDoc tag** — use only on technically reachable members without `private` / `protected`; never add it to members that are already hidden by accessibility. `@internal` does not replace explicit `public` when a member is still technically reachable.
...
- **`public`** is an explicit opt-in for members that intentionally expose an external API across class boundaries; on Angular classes, reachable members should use explicit `public` unless they are intentionally hidden behind `private` / `protected`
- **Never `static`**; **never `protected`** unless absolutely necessary — template usage (the member is referenced in the HTML template file) **or `host:` binding expressions** in the `@Component` / `@Directive` decorator count as absolutely necessary. Those members should use an explicit accessibility modifier; lint may auto-fix to `public`, but `protected` remains valid when they are intentionally template-only.
