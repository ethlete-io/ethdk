...
| `session()`                        | `{ status, endCause, executionState }` as one signal - see [Is there a session?](#is-there-a-session).                                                                                                |
...
`sessionStatus()` is `'authenticated'` exactly while `isAuthenticated()` is `true`, so it is the one to read. Logic that reacts to more than the status reads `session()` - `{ status, endCause, executionState }` as one signal. The separate signals agree on every synchronous read, but `toObservable()` of each emits them one at a time, so a `combineLatest` over them sees `{ type: 'login', state: 'success' }` next to a stale `isAuthenticated()` of `false`:

```ts
toObservable(auth.session)
  .pipe(filter(({ executionState }) => executionState?.type === 'login' && executionState.state === 'success'))
  .subscribe(/* … */);
```
...
| Feature                      | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `withPersistentAuth`         | Cookie-backed "remember me" auto-login (encrypted token storage; cookie `etAuth`, 30 days, `sameSite: 'lax'` by default). It calls `tryLogin()` itself during setup - no app initializer needed; the attempt surfaces as `executionState()` with `type: 'autoLogin'` and as [`sessionStatus()`](#is-there-a-session) `'restoring'`. `autoLogin.buildArgs` defaults to the refresh query's `buildArgs` when `autoLogin.queryKey` names it. |
| `withTokenExpirationWarning` | `isExpiringSoon` / `expiresIn` signals (default threshold 5 minutes). It reads the claim named by `expiresInPropertyName`, which defaults to the refresh query's (and to `'exp'` without one).                                                                                                                                                                                                                                            |
...
| `withTokenRevocation`        | Calls a revocation query - by default automatically on logout. `revokeOn` limits the automatic call to some session end causes, for example `['user']` when the API ends the sessions on all devices. `bearer: true` sends the revoked access token as `Authorization: Bearer <token>`.                                                                                                                                                   |
...
| Type                                                                                                                                                        | What it is                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CreateBearerAuthProviderConfig`                                                                                                                            | The argument of `createBearerAuthProvider`.                                                                                                                                                                                                                                                                          |
| `AnyCreateBearerAuthProviderResult`                                                                                                                         | An alias of `BearerAuthProviderRef` with its generics erased - what the secure creator templates take.                                                                                                                                                                                                               |
| `BearerAuthProviderRef`                                                                                                                                     | The provider definition it returns. `BearerAuthProviderOf<typeof ref>` recovers the injected provider's real type - prefer it over `AnyBearerAuthProvider`.                                                                                                                                                          |
| `AuthQueryConfig`, `TokenRefreshQueryConfig`                                                                                                                | The second argument of `withAuthenticationQuery` and `withRefreshQuery`. Their results are an `AuthQueryBuilder` / `TokenRefreshQueryBuilder`, both `AnyQueryBuilder`s.                                                                                                                                              |
| `BearerAuthProviderTokens`                                                                                                                                  | `{ accessToken, refreshToken }` - what an `extractTokens` returns, i.e. what a login/refresh response is reduced to.                                                                                                                                                                                                 |
| `RefreshFailure`                                                                                                                                            | What `onRefreshFailure` receives: the `error`, plus the calls that end or keep the session.                                                                                                                                                                                                                          |
| `BearerAuthSessionStatus`, `BearerAuthSessionEndCause`, `BearerAuthSession`                                                                                 | The [`sessionStatus()`](#is-there-a-session) and [`sessionEndCause()`](#why-the-session-ended) unions, and the [`session()`](#is-there-a-session) value.                                                                                                                                                             |
| `BearerAuthExecutionState`                                                                                                                                  | The [`executionState()`](#execution-state) union, discriminated on `type`/`state`. Its members are `BearerAuthExecutionStateLoading`, `BearerAuthExecutionStateSuccess`, `BearerAuthExecutionStateError`, `BearerAuthExecutionStateLogout` and `BearerAuthExecutionStateTokenSeed`.                                  |
