...
- **HTTP error pipes** - `ParseHttpErrorCodeToTitleEnPipe`, `ParseHttpErrorCodeToTitleDePipe`, `ParseHttpErrorCodeToMessageEnPipe` and `ParseHttpErrorCodeToMessageDePipe` translate HTTP status codes into a human-readable title and message (English/German). The same tables are exported as plain functions taking an `HttpStatusCode` - `parseHttpErrorCodeToTitleEn`, `parseHttpErrorCodeToTitleDe`, `parseHttpErrorCodeToMessageEn` and `parseHttpErrorCodeToMessageDe` - for use outside a template. To render a failed query, reach for [`<et-query-error>`](/components/query-error) first - it already composes a title, the message or violation list and a retry from these tables.
- **`QUERY_VERSION`** - the version of `@ethlete/query` the build was cut from, as a string. Generated from the package's own `package.json`; the devtools panel shows it under About.
