...
- `abort()` - stops the execution in flight, including a retry it is waiting out, and returns `true`; with nothing in flight it returns `false` and changes nothing. The aborted execution sets no response and no error: `loading()` ends, and `response()`, `error()` and `executionState()` go back to what they were before it started - `null` if the query never settled. There is no cancelled state. `args()` and `lastTimeExecutedAt()` keep describing the aborted run, and the next `execute()` runs as usual. The request is shared by every query bound to the same cache key, so they stop loading too. An aborted execution completes an `executeUntilSettled$` without a value, and resolves an `executeUntilSettled` with a snapshot that has neither response nor error and whose `latestHttpEvent()` is `{ type: 'cancel' }`. Aborting a [`withLongPolling`](/query/features) round ends the chain until a new `withArgs` value or `execute()` starts it again; a pending round between two requests is not in flight, so `abort()` leaves it alone.
- `createSnapshot()` - a frozen copy of the current state with an `isAlive` signal. Useful for "the request I started", untouched by later executions. The standalone `executeUntilSettled$(query, executeArgs?)` combines both: a cold Observable that executes on subscribe, emits the settled snapshot and completes; unsubscribing before it settles aborts the execution like `abort()`. `executeUntilSettled(query, executeArgs?)` is its Promise form for APIs that need an `async` function, like a signal-forms [`submit()` action](/query/errors#mapping-violations-onto-signal-forms). For a chain of dependent mutations built on top of it, see [Dependent queries](/query/dependent-queries).
...
const staleResults = computed(() => {
  const state = search.executionState();

  return state?.type === 'loading' && state.hasCachedResponse ? state.cachedResponse : null;
});
```
