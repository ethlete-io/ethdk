...
| Devtools        | `<et-query-devtools>` component showing live queries and auth state (`provideQueryClientForDevtools`).                                                           | `provideQueryDevtools()` from `@ethlete/query` + [`<et-query-devtools />`](/query-devtools/) from `@ethlete/query-devtools`     |
...
| `query.poll({ interval, takeUntil })`                                | [`withPolling({ interval })`](/query/features#withpolling)                                                                                              |
...
| `provideQueryClientForDevtools`                                      | `provideQueryDevtools()` from `@ethlete/query` + [`<et-query-devtools />`](/query-devtools/) (registers every client at once)                           |
