...
| **Batches**   | Each [`createQueryBatch`](/query/batching#inspecting-a-run-in-the-devtools) run: status, a progress bar, the measured throughput and time remaining, the concurrency it was configured with, and one row per item - resolved route, args in, response or error out - with the failures listed first. A row opens the item's own query in a split-view drawer while that query is still kept.                                                                                                                                                                                                                                                                                                          |
...
Every tab that opens a drawer - Stacks, Sequences, Batches, Forms, Timeline, Events -
splits along the same divider the Queries tab uses: **drag it** to resize the pair,
**double-click** it to go back to the default. Docked to a side the panel stacks the
two panes instead, and the same divider sizes them by height; both sizes are kept, so
switching between a bottom and a side dock does not lose either. Which query each of
those drawers is showing is its own - opening one from the Timeline never changes what
the Forms drawer holds - and each is [persisted](#persistence) across a reload.
