...
In edit mode every item renders an actions component in its top corner - by default `et-grid-item-default-actions`, a small toolbar with a remove button. Replace it globally via `provideGridConfig({ actionsComponent: MyActionsComponent })`; the component receives the item's `itemId` and `data` as inputs and can call the grid's `removeItem()` (see the `GridItemActionsComponent` type). The default remove button goes through the item, so a projected item's `(remove)` output fires for it just as for the keyboard shortcut; a custom actions component that calls the grid's `removeItem()` directly bypasses that output. As with a registered widget, `data` can be typed with your own payload type - declare `itemId = input.required<string>()` and `data = input.required<MyPayload>()`, no cast needed. `et-grid-item-toolbar` is the styled toolbar shell (`--et-grid-item-toolbar-*` tokens) you can reuse in a custom actions component.
