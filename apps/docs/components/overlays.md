...
A pane near a side edge keeps its placement and shifts along that edge: `flip` changes sides only for overflow on the placement's own axis, and `shift` handles the rest. With `shift: false`, `flip` also changes sides for cross-axis overflow.
...
- **Passive overlays**: `passive: true` marks an overlay that never owns the interaction - a tooltip, a hover card. The runtime skips it while it looks for the top layer, so the overlay under it keeps answering <kbd>Escape</kbd>, an outside press and its focus trap for as long as the passive one shows. `etTooltip` sets it.
