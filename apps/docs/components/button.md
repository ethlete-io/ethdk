...
- `disabled` and `loading` both make the button **inactive**: `aria-disabled`, no pointer events, and a click that is blocked before your own `(click)` handler runs - keyboard activation included.
