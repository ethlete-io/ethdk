...
| `ET3406` | A match-to-match relation couldn't be resolved (match counts don't line up). | Give each round the match count the mode implies for the round it feeds - an empty round is linked over, so the counts either side of it are the ones that must line up. Or declare `homeSource`/`awaySource` and skip the count math. |
