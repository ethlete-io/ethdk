...
- **Boundaries.** `et-checkbox` and `et-switch` carry this concept as their platform-named `indeterminate` input (see above) rather than `mixed` — both render a first-class indeterminate state. `et-checkbox` reflects it as `aria-checked="mixed"`, so it is announced; `et-switch` shows it visually (thumb parked mid-track, dashed accent) but keeps `aria-checked` boolean, since `role="switch"` has no `"mixed"` in ARIA — reach for `et-checkbox` when the state itself must reach assistive tech. `et-otp-input` and the rich text editors are not bulk-edit fields.
