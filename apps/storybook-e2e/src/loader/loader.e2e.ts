...
function setToken(locator: Locator, token: string, value: string): Promise<void> {
  return locator.evaluate((el, [name, next]) => (el as HTMLElement).style.setProperty(name, next), [token, value]);
}

function setParentToken(locator: Locator, token: string, value: string): Promise<void> {
  return locator.evaluate(
    (el, [name, next]) => (el.parentElement as HTMLElement).style.setProperty(name, next),
    [token, value],
  );
}
