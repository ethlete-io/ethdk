...
test.describe('split-button / focus', () => {
  ...
  test('the group is exactly two tab stops, action before trigger', async ({ page }) => {
    ...
    const [first, second, third] = await tabSequence(page, 3);

    expect(first.text).toContain('Save changes');
    expect(second.name).toBe('More save options');
    expect(third.text).toContain('Save changes');
