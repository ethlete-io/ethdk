...
import { at, expectTouchMode, focusedDescriptor, openStory, tap } from '../support';
...
test.describe('standings / structure', () => {
  ...
  test('the highlighted row is aria-current and announces the highlightedRow label', async ({ page }) => {
    ...
    await expect(highlighted.locator('.et-match-participant-name')).toHaveText(at(PARTICIPANTS, 1));
