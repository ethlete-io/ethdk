...
const liveLine = html`
  <span class="live">
    <span class="dot"></span>
    <b>${LIVE.name}</b>
    <span class="live__detail">is live now</span>
  </span>
`;

const subRow = (rule: ControlRule) => {
  if (rule === 'whole-row') {
    return html` <button class="sub sub--whole" type="button">${liveLine}<span class="grow"></span>${grid}</button> `;
  }
  ...
  if (rule === 'bare-icon') {
    return html`
      <nav class="sub">
        ${liveLine}
        <span class="grow"></span>
        <button class="icon icon--map" type="button" aria-label="All of the competition">${grid}</button>
      </nav>
    `;
  }
 ...
 */
export const mapControl = ({ rule }: { rule: ControlRule }) =>
  drawing({
    body: html`
