...
/** How the row stops a very long competition name from taking the width. */
export type NameRule = 'short' | 'cap' | 'entry';
...
const links = (rule: NameRule) => html`
  <div class="links">
    ${
      rule === 'entry' &&
      html`<a class="name-entry ${CURRENT === 'Overview' ? 'is-current' : ''}">${COMPETITION.name}</a>`
    }
    ${VISIBLE_PAGES.map(
      (page) =>
        html`<a class="${page === CURRENT && rule !== 'entry' ? 'is-current' : ''}"
          >${rule === 'entry' && page === 'Overview' ? 'About' : page}</a
        >`,
    )}
    ${overflowButton}
  </div>
...
const name = (rule: NameRule) =>
  rule === 'entry'
    ? ''
    : html`<span class="name">${rule === 'short' ? COMPETITION.shortName : COMPETITION.name}</span>
        <span class="rule"></span>`;

/**
 * The chosen row, adapted: the overflow menu is one more entry in the strip and the stage control
 * sits at the far right. Only `rule` changes: how the row carries a 72-character name.
 */
export const nameWidth = ({ rule }: { rule: NameRule }) =>
  drawing({
    body: html`
