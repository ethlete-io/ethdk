...
const thumb = (variant: Variant) => html`
  <div class="thumb ${variant.key === LARGE && 'on'} ${variant.key === HOVERED && 'pointer'}">
    <div class="thumb-pic ${variant.verdict === 'rejected' && 'dim'}">
      ${mini(variant)}
      ${
        variant.key === HOVERED &&
        html`<div class="band">${VERBS.map((verb) => html`<span class="chip">${verb}</span>`)}</div>`
      }
    </div>
