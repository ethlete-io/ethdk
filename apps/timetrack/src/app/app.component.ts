...
@Component({
  ...
  template: `
    <!--
      Nothing at all until the host has answered whether the window is locked, and the lock view alone
      while it is. A view mounted beside it would read the day into the window it is there to cover.
    -->
    @if (!lock.ready()) {
      <div class="h-dvh" data-tauri-drag-region="deep"></div>
