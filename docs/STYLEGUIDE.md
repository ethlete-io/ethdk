...
| No property bindings for static strings or coercing component booleans (`[etIcon]="'foo'"`, `[isReadonly]="true"`) - use static attributes | `@angular-eslint/template/prefer-static-string-properties`, `ethlete/prefer-static-boolean-properties`                                                                                                              |
| A `<form>` handles its own submission (`[etForm]`, `[formRoot]`, `(ngSubmit)`, `(submit)`)                                                 | `ethlete/require-form-submit`                                                                                                                                                                                       |
| No `interface` - use `type` (module augmentations stay interfaces)                                                                         | `ethlete/consistent-type-definitions`                                                                                                                                                                               |
| No `enum` and no `const enum` - use an `as const` object + derived union                                                                   | `ethlete/no-enum`                                                                                                                                                                                                   |
...
| No blank lines between consecutive imports                                                                                                 | `ethlete/no-empty-newlines-between-imports`                                                                                                                                                                         |
...
| Injected providers `private` by default, `protected` when template/host-visible or inherited; explicit accessibility on reachable members  | `ethlete/inject-member-accessibility`, `ethlete/template-member-accessibility`                                                                                                                                      |
| Injected `ElementRef` values carry the native element type                                                                                 | `ethlete/no-typed-injected-element-ref`                                                                                                                                                                             |
| No `inject(X).member` chaining, pure member aliases or trivial non-contract wrappers                                                       | `ethlete/no-inject-chain`, `ethlete/no-member-alias`, `ethlete/no-trivial-wrapper-method`                                                                                                                           |
| No provably unread private/protected class members                                                                                         | `ethlete/no-unused-class-member`                                                                                                                                                                                    |
...
| Angular ≤21 components declare `OnPush`; Angular 22+ components omit the now-redundant metadata                                            | `ethlete/require-on-push-change-detection`, `ethlete/no-redundant-on-push-change-detection`                                                                                                                         |
| No redundant `standalone: true` or empty Angular metadata arrays                                                                           | `ethlete/no-standalone-flag`, `ethlete/no-empty-angular-metadata-arrays`                                                                                                                                            |
...
| No imports from maintenance-mode CDK or legacy query APIs when migration enforcement is enabled                                            | `ethlete/no-cdk-import`, `ethlete/no-legacy-query-import`                                                                                                                                                           |
