...
const generateDeclaredMatchRelations = <TRoundData, TMatchData>(options: {
  ...
  const relations: BracketMatchRelation<TRoundData, TMatchData>[] = [];

  for (const match of bracket.matches.values()) {
    const previousMatches = (previousByMatchId.get(match.id) ?? [])
      .map((id) => bracket.matches.get(id as BracketMatchId))
      .filter((candidate): candidate is BracketMatch<TRoundData, TMatchData> => candidate !== undefined);
    const nextMatchId = nextByMatchId.get(match.id);
    const nextMatch = nextMatchId ? bracket.matches.get(nextMatchId as BracketMatchId) : undefined;

    if (previousMatches.length === 0 && nextMatch) {
      relations.push({
        type: 'nothing-to-one',
        currentMatch: match,
    ...
    } else if (previousMatches.length === 1 && nextMatch) {
      ...
      relations.push({
        ...
        currentRound: match.round,
        previousMatch,
        previousRound: previousMatch.round,
        nextMatch,
        nextRound: nextMatch.round,
      });
    } else if (previousMatches.length >= 2 && nextMatch) {
      const previousUpperMatch = previousMatches[0];
      const previousLowerMatch = previousMatches[1];

      if (!previousUpperMatch || !previousLowerMatch) continue;
      ...
      relations.push({
        ...
        currentRound: match.round,
        previousUpperMatch,
        previousUpperRound: previousUpperMatch.round,
        previousLowerMatch,
        previousLowerRound: previousLowerMatch.round,
        nextMatch,
        nextRound: nextMatch.round,
      });
