import { BracketRoundSwissData, generateBracketRoundSwissGroupMaps, Bracket } from '../../linked';
import {
  BracketComponents,
...
export const createSwissGrid = <TRoundData, TMatchData>(
  ...
  if (!swissGroupMaps)
    throw new BracketRuntimeError(
      ...
      'Unable to generate Swiss groups for the provided bracket data',
    );
  ...
  for (const [groupMapIndex, groupMap] of groupMaps.entries()) {
    ...
    for (const [groupIndex, group] of Array.from(groupMap.groups.values()).entries()) {
      ...
      if (options.roundHeaderHeight > 0) {
        ...
        if (!firstMatchRound)
          throw new BracketRuntimeError(BRACKET_ERROR_CODES.SWISS_GROUP_EMPTY, 'No rounds found in Swiss group');
