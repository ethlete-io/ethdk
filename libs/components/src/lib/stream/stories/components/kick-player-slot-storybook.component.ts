...
@Component({
  ...
  template: `
    <div class="bg-neutral-900 rounded-lg overflow-hidden">
      ...
      @if (page() === 'a') {
        <div class="p-5">
          <p class="mb-4 text-sm font-semibold text-white">Page A — {{ channel() }}</p>

          <et-kick-player-slot #slotA [channel]="channel()" class="block relative w-full aspect-video bg-black rounded mb-4" />
      ...
      @if (page() === 'b') {
        <div class="p-5">
          <p class="mb-4 text-sm font-semibold text-white">Page B — asmongold247</p>

          <et-kick-player-slot #slotB class="block relative w-full aspect-video bg-black rounded mb-4" channel="asmongold247" />
