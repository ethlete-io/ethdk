...
@Component({
  ...
  template: `
    <div class="bg-neutral-900 rounded-lg overflow-hidden">
      ...
      @if (page() === 'a') {
        <div class="p-4">
          <p class="mb-3 text-small font-bold text-white">Page A — {{ channel() }}</p>

          <et-kick-player-slot #slotA [channel]="channel()" class="block relative w-full aspect-video bg-black mb-3" />
      ...
      @if (page() === 'b') {
        <div class="p-4">
          <p class="mb-3 text-small font-bold text-white">Page B — asmongold247</p>

          <et-kick-player-slot #slotB class="block relative w-full aspect-video bg-black mb-3" channel="asmongold247" />
