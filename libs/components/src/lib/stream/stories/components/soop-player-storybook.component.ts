import { JsonPipe } from '@angular/common';
import { Component, ViewEncapsulation, input, viewChild } from '@angular/core';
import { SoopPlayerSlotComponent } from '../../platform/soop/soop-player-slot.component';
import { STREAM_IMPORTS, STREAM_SOOP_IMPORTS } from '../../stream.imports';

@Component({
  selector: 'et-sb-soop-player',
  template: `
    <et-soop-player-slot [userId]="userId()" [videoId]="videoId()" class="block w-full max-w-4xl aspect-video" />

    <div class="mt-6 bg-neutral-900 rounded-lg p-4">
      <p class="text-xs font-mono text-neutral-400 mb-2">State</p>
      <pre class="bg-neutral-950 rounded p-3 text-xs font-mono text-neutral-300 m-0 overflow-auto">{{
        player().slotDirective.slot.currentState() | json
      }}</pre>
    </div>
  `,
  encapsulation: ViewEncapsulation.None,
  imports: [STREAM_IMPORTS, STREAM_SOOP_IMPORTS, JsonPipe],
})
export class SoopPlayerStorybookComponent {
  public userId = input<string | null>('kbsnews');
  public videoId = input<string | null>(null);
  protected player = viewChild.required(SoopPlayerSlotComponent);
}
