...
@Component({
  ...
  template: `
    <div class="bg-neutral-900 rounded-lg overflow-hidden">
      ...
      @if (page() === 'a') {
        <div class="p-4">
          <p class="mb-3 text-small font-bold text-white">Page A — {{ src() }}</p>

          <et-twitch-player-slot #slotA [src]="src()" class="block relative w-full aspect-video bg-black mb-3" />
      ...
      @if (page() === 'b') {
        <div class="p-4">
          <p class="mb-3 text-small font-bold text-white">Page B — Monstercat</p>

          <et-twitch-player-slot #slotB class="block relative w-full aspect-video bg-black mb-3" src="monstercat" />
