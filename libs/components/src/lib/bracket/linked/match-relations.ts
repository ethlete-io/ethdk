...
const createNothingToOneRelation = <TRoundData, TMatchData>(params: {
  ...
  if (!nextMatch) throw new RuntimeError(BRACKET_ERROR_CODES.MATCH_RELATION_INVALID, 'Next round match not found');
...
const createOneToNothingOrTwoToNothingRelation = <TRoundData, TMatchData>(params: {
  ...
  if (!previousUpperMatch)
    throw new RuntimeError(BRACKET_ERROR_CODES.MATCH_RELATION_INVALID, 'Previous round match not found');

  if (previousUpperRoundMatchPosition !== previousLowerRoundMatchPosition) {
    if (!previousLowerMatch)
      throw new RuntimeError(BRACKET_ERROR_CODES.MATCH_RELATION_INVALID, 'Previous lower round match not found');
...
const createOneToOneOrTwoToOneRelation = <TRoundData, TMatchData>(params: {
  ...
  if (!nextMatch) throw new RuntimeError(BRACKET_ERROR_CODES.MATCH_RELATION_INVALID, 'Next round match not found');
  if (!previousUpperMatch)
    throw new RuntimeError(BRACKET_ERROR_CODES.MATCH_RELATION_INVALID, 'Previous upper round match not found');
  if (!previousLowerMatch)
    throw new RuntimeError(BRACKET_ERROR_CODES.MATCH_RELATION_INVALID, 'Previous lower round match not found');
