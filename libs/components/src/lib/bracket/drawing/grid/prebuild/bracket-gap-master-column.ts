...
export const createBracketGapMasterColumn = <TRoundData, TMatchData>(
  ...
  if (!lastMasterColumn) {
    throw new RuntimeError(BRACKET_ERROR_CODES.GRID_INVALID, 'No last master column found in existing master columns');
  }
  ...
  for (const section of lastMasterColumn.sections) {
    ...
    if (!lastSubColumn) {
      throw new RuntimeError(BRACKET_ERROR_CODES.GRID_INVALID, 'No last sub column found in section');
    }
