...
export type CreateDatePickerOverlayOptions = {
  ...
  /** Runs once the pane is gone; `byOutsidePointer` distinguishes a deliberate click elsewhere. */
  onAfterClosed?: (byOutsidePointer: boolean) => void;
};
    ...
    currentRef
      ...
      .pipe(
        ...
        tap(() => {
          ...
          const byOutsidePointer = closedByOutsidePointer;

          closedByOutsidePointer = false;
          options.onAfterClosed?.(byOutsidePointer);
        }),
      )
      .subscribe();
  };
