...
describe('tableCsvRowsFromPages', () => {
  it('walks pages until one comes back empty and concatenates them', async () => {
    ...
    await expect(firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage })())).resolves.toEqual([
      { id: 1 },
      { id: 2 },
      { id: 3 },
      { id: 4 },
  ...
  it('fetches one page at a time, in order', async () => {
    ...
    await firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage })());
  ...
  it('takes a custom `hasMore`, so a backend that says so directly needs no empty trailing page', async () => {
    ...
    await expect(
      firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage, hasMore: (_, n) => n < 2 })()),
  ...
  it('starts at `initialPage`, for a 0-based API', async () => {
    ...
    await firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage, initialPage: 0 })());
  ...
  it('stops at `maxPages` even when `hasMore` never gives up', async () => {
    ...
    await expect(firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage, maxPages: 3 })())).resolves.toHaveLength(3);
  ...
  it('reads an observable page to its first emission', async () => {
    ...
    await expect(firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage })())).resolves.toEqual([{ id: 1 }]);
  });
  ...
  it('treats an observable page that completes without emitting as the end', async () => {
    ...
    const rows = firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage })());
  ...
  it('lets a failing page reject rather than writing a short file', async () => {
    ...
    await expect(firstValueFrom(tableCsvRowsFromPages<Row>({ fetchPage })())).rejects.toThrow('502');
  });
});
