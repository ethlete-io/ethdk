...
@Component({
  template: `
    <et-table [columns]="columns()" [data]="data()">
      <et-table-filters />
    </et-table>
  `,
...
class HostComponent {
  public columns = signal<AnyTableColumn<Person>[]>([]);
  public data = signal<Person[]>(PEOPLE);

  public table = viewChild.required<TableComponent<Person>>(TableComponent);
  public filters = viewChild.required(TableFiltersComponent);
}

const create = (cols: AnyTableColumn<Person>[]) => {
...
type FilterInternals = {
  optionsFor: (column: AnyTableColumn<Person>) => { label: string; value: unknown }[];
  setSearchQuery: (column: AnyTableColumn<Person>, query: string) => void;
  loading: (column: AnyTableColumn<Person>) => boolean;
  hasSearch: (column: AnyTableColumn<Person>) => boolean;
};
...
describe('TableFiltersComponent', () => {
  ...
  it('throws a labelled error when used outside a table', () => {
    @Component({
      template: `<et-table-filters />`,
      imports: [TABLE_FILTER_IMPORTS],
    })
    class OrphanComponent {}

    expect(() => TestBed.createComponent(OrphanComponent)).toThrow(
      expect.objectContaining({ code: TABLE_ERROR_CODES.FEATURE_OUTSIDE_TABLE }) as unknown as RuntimeError,
    );
  });
});
