 ...
 * without both. The width fake only takes effect once `measure()` is first called: `GridDirective`
 * gates its one-shot animation enablement on its first measurement, so a spec asserting the
 * pre-measurement state must still see a real, unmeasured container.
 */
