...
type IconsConfig = {
  ...
  source?: string;
  /** Variant used for shorthand string entries and entries without an explicit variant. Defaults to 'solid'. */
...
export default async function generate(tree: Tree, schema: GeneratorSchema) {
  ...
  if (!tree.exists(configPath)) {
    ...
    logger.log(
      `  { "source": "auto", "defaultVariant": "solid", "icons": ["plus", { "name": "shield", "variants": ["light", "solid"] }] }\n`,
    );
    return;
  }
...
function normalizeIcons(entries: IconEntry[], defaultVariant: string): ResolvedIcon[] {
...
function generateIconsFile(icons: (ResolvedIcon & { data: string })[], schema: GeneratorSchema, source: string): string {
...
function generateTypesFile(icons: ResolvedIcon[], schema: GeneratorSchema): string {
  const names = [...new Set(icons.map((i) => i.name))].sort().map((n) => `'${n}'`).join(' | ');
  const variants = [...new Set(icons.map((i) => i.variant))].sort().map((v) => `'${v}'`).join(' | ');
