...
export const emitHookScripts = (options: { hooks: string[]; hooksDir: string; version: string }): EmittedFile[] => {
  const { hooks, hooksDir, version } = options;
  ...
  return hooks.flatMap((name) => {
    ...
    return [{ path: `${hooksDir}/${definition.file}`, contents: withBanner(raw, version) }];
  });
};
