...
Where this repo installs the `context-warning` hook, it turns handoff mode on at 70% of the
token budget, then repeats a short note with the remaining budget on every prompt after that. Handoff mode is not an
