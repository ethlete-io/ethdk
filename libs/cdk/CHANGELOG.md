...
- [#3037](https://github.com/ethlete-io/ethdk/pull/3037) [`8122287`](https://github.com/ethlete-io/ethdk/commit/8122287535a4661816ab7f930891aac8efd68ef1) Thanks [@github-actions](https://github.com/apps/github-actions)! - Bracket: fix the missing losers-bracket → grand-final connector in a double-elimination bracket that has no bracket-reset (reverse) final. The `two-to-nothing` final relation resolved and stored its lower input from the winners round instead of the losers round, so the line was never drawn. Brackets that include a reverse final were unaffected.
...
- [#3008](https://github.com/ethlete-io/ethdk/pull/3008) [`6b9c784`](https://github.com/ethlete-io/ethdk/commit/6b9c784d4d7fa19fd4c2c1ee48998d8256d3b748) Thanks [@TomTomB](https://github.com/TomTomB)! - Remove all `@angular/animations` `trigger()` usage from the CDK. The accordion and inline tab body were the last two components relying on the legacy animations engine; the accordion now uses a pure CSS `grid-template-rows` transition and the inline tab body drives its portal lifecycle directly. Consumer apps can now drop `provideAnimations()`/`provideNoopAnimations()` entirely, which restores synchronous route-view removal under zoneless change detection and removes the brief old/new view overlap flash on navigation ([#3005](https://github.com/ethlete-io/ethdk/issues/3005)).
