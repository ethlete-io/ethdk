...
export class QueryErrorDirective {
  ...
  readonly errorList = computed(() => {
    ...
    const [onlyItem] = errorList.items;

    if (errorList.items.length === 1 && onlyItem) {
      const compareString = (str: string) => str.toLowerCase().replace(/\s/g, '').replace(/\.|,/g, '');

      if (compareString(onlyItem.message) === compareString(errorList.title)) {
        ...
        onlyItem.message = defaultErrorMessage;
      }
    }
