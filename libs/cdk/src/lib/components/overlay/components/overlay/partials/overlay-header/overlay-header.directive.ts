...
export class OverlayHeaderDirective implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this._overlayRef._updateLayout({ hasHeader: true });
  }

  ngOnDestroy() {
    this._overlayRef?._updateLayout({ hasHeader: false });
  }
}
