...
The template of individual pages should look like the one from the "Overlay with layout" example.
...
The filter funcionalities can be implemented by using the `provideFilterOverlayConfig` function inside the overlay's host component's `providers` array.

```ts
@Component({
  selector: 'example-overlay-host',
  providers: [
