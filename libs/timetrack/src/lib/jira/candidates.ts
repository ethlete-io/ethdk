...
/**
 * The open issues in one project that a new ticket could roll up to, most recently active first.
 *
 * Only open ones: a done Story is not something today's work belongs under, and offering it is how a
...
export const fetchJiraParentCandidates$ = (options: {
  ...
  return searchJiraIssues$({
    ...
    options: { pageSize: options.limit ?? DEFAULT_PARENT_CANDIDATE_LIMIT, maxPages: 1 },
